...
public class CountryRepo {
    ...
    public ArrayList<String> getCountryList2()
    {
        //Open connection to read only
        SQLiteDatabase db = dbHandler.getReadableDatabase();
        String selectQuery =  "SELECT  " +
        ...
        if (cursor.moveToFirst()) {
            do {
                countryList.add(cursor.getString(cursor.getColumnIndex(Country.KEY_NAME)));
            } while (cursor.moveToNext());
        }
    ...
    public long getIdbyName(String name) {
        ...
        String selectQuery =  "SELECT  " +
                ...
                " FROM " + Country.TABLE
                + " WHERE " +
                Country.KEY_NAME + "=?";// It's a good practice to use parameter ?, instead of concatenate string
